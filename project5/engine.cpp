...
Engine::Engine() :
  ...
  normalSprites(),
  hudFlag(false),
  player2(new ShootingSprite("sasukeRun")),
  shootFlag(false)
  
{
  ...
  Viewport::getInstance().setObjectToTrack(player2);
      ...
      if(event.type == SDL_KEYDOWN) {
        ...
        if ( keystate[SDL_SCANCODE_P] ) {
         ...
         if ( keystate[SDL_SCANCODE_SPACE] ) {
          if(shootFlag){
            static_cast<ShootingSprite*>(player2)->shoot();
          }else{
          static_cast<ShootingSprite*>(player2)->shootRight();
          }
        }
